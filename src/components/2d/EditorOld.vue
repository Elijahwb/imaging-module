<template>
  <div class="image-editor">
    <ImageEditor
      class="image"
      ref="imageEditor"
      :include-ui="useDefaultUI"
      :options="options"
    ></ImageEditor>
    <button class="btn btn-danger pb-36" @click="startDrawing">Mark</button>
    <button class="btn btn-primary pb-36" @click="addText('Type something...')">
      Add Text
    </button>
    <button class="btn btn-secondary pb-36 pl-10" @click="startCropping()">
      Start Cropping
    </button>
    <button class="btn btn-primary pb-36" @click="crop()">crop</button>
    <label for="input-image-file"
      ><span class="btn btn-dark pb-36">Load Image</span></label
    >
    <input
      class="input-image"
      type="file"
      accept="image/*"
      id="input-image-file"
      @change="loadLocalFile"
    />
    <span class="btn btn-success pb-36" @click="zoomChange('in')">Zoom +</span>
    <span class="btn btn-secondary pb-36" @click="zoomChange('out')"
      >Zoom -</span
    >
  </div>
</template>

<script>
import "tui-color-picker/dist/tui-color-picker.css";
import "tui-image-editor/dist/tui-image-editor.css";
import demo from "@/assets/xrays/6.jpeg";
import { ImageEditor } from "@toast-ui/vue-image-editor";

export default {
  name: "EditorOld",
  components: {
    ImageEditor,
  },
  data() {
    return {
      getNumber: (value) => Number(value.replace("px", "")),

      resizeElement: (element, percentage, props = [], values = []) => {
        console.log({ element, percentage, props, values });
        props.forEach((key, idx) => {
          element.style[key] = values[idx] * percentage + "px";
        });
      },
      zoom: 1,
      maxZoom: 2,
      height: 0,
      width: 0,
      minZoom: 0.5,
      loaded: false,
      useDefaultUI: false,
      options: {
        // for tui-image-editor component's "options" prop
        cssMaxWidth: 700,
        cssMaxHeight: 500,
        selectionStyle: {
          cornerSize: 20,
          rotatingPointOffset: 70,
        },
        includeUI: {
          loadImage: {
            path: "data:image/png;base64,",
            name: "Blank",
          },
          menuBarPosition: "right",
        },
      },
    };
  },
  mounted() {
    // let ed = (this.$refs['imageEditor'] as any).invoke()
    const editor = this.$refs.imageEditor.getRootElement();
    console.log({ editor });

    // this.$refs.imageEditor
    //   .loadImageFromURL(
    //     "https://www.biltmoreperiodontics.com/wp-content/uploads/2021/06/teeth-in-skeleton-lowrez-1024x626.jpg",
    //     "SampleImage"
    //   )
    //   .then((objectProps) => {
    //     console.log("Image Loaded");
    //     console.log(objectProps.id);
    //   })
    //   .catch((error) => {
    //     console.log({ error });
    //   });
    // this.resizeEditor();
    this.loadImage();
  },
  methods: {
    loadImage() {
      this.$refs.imageEditor
        .invoke(
          "loadImageFromURL",
          demo,
          // "data:image/png;base64,",
          "My sample image"
        )
        .then((result) => {
          this.resizeEditor(result);
          // console.log(result);
          // this.$refs.imageEditor.invoke("rotate", 10).then((status) => {
          // console.log({ status });
          // });
        });
    },
    resizeEditor(dimensions) {
      this.$refs.imageEditor
        .invoke("resize", {
          imageSize: {
            oldWidth: dimensions.oldWidth,
            oldHeight: dimensions.oldHeight,
            newWidth: dimensions.newWidth,
            newHeight: dimensions.newHeight,
          },
          uiSize: { width: dimensions.newWidth, height: dimensions.newHeight },
        })
        .then((result) => {
          console.log(result);
        });
    },
    addText(text) {
      this.$refs.imageEditor.invoke("stopDrawingMode");
      this.$refs.imageEditor.invoke("addText", text, {
        styles: {
          fill: "red",
          fontSize: 20,
          fontWeight: "bold",
        },
        position: {
          x: 10,
          y: 10,
        },
      });
    },
    rotateClockwise() {
      this.$refs.imageEditor.invoke("rotate", 10).then((status) => {
        console.log({ status });
      });
    },
    rotateAntiClockwise() {
      this.$refs.imageEditor.invoke("rotate", -10).then((status) => {
        console.log({ status });
      });
    },
    addShape() {
      this.$refs.imageEditor
        .invoke("addShape", "rect", {
          fill: "red",
          stroke: "blue",
          strokeWidth: 3,
          width: 100,
          height: 200,
          left: 10,
          top: 10,
          isRegular: true,
        })
        .then((status) => {
          console.log({ status });
        });
    },
    startCropping() {
      let crop = this.$refs.imageEditor.invoke("startDrawingMode", "CROPPER");
      console.log({ crop });
      // this.$refs.imageEditor.invoke("getCropzoneRect").then((data) => {
      //   console.log({ data });
      //   // this.$refs.imageEditor.invoke("crop", data).then((status) => {
      //   //   console.log({ status });
      //   // });
      // });
    },
    cameraZoom() {
      this.$refs.imageEditor.invoke("zoom", {}, 1, 1, 2.5).then((status) => {
        console.log({ status });
      });
    },

    startDrawing() {
      this.$refs.imageEditor.invoke("startDrawingMode", "FREE_DRAWING", {
        width: 5,
        color: "rgba(255, 0, 0, 0.5)",
      });
    },
    showActions() {
      let actions = this.$refs.imageEditor.invoke("getActions");
      console.log({ actions });
      let mode = this.$refs.imageEditor.invoke("getDrawingMode");
      console.log({ mode });
      let cropRect = this.$refs.imageEditor.invoke("getImageName");
      console.log({ cropRect });
      let draw = this.$refs.imageEditor.invoke("setDrawingShape", "rect", {
        fill: {
          type: "filter",
          filter: [{ blur: 0.3 }, { pixelate: 20 }],
        },
        width: 100,
        height: 200,
      });
      console.log({ draw });
      let DrawingMode = this.$refs.imageEditor.invoke(
        "startDrawingMode",
        "LINE_DRAWING",
        {
          width: 10,
          color: "rgba(255,0,0,0.5)",
          arrowType: {
            tail: "chevron", // triangle
          },
        }
      );
      console.log({ DrawingMode });
    },
    loadLocalFile(event) {
      var file;
      file = event.target.files[0];
      this.$refs.imageEditor
        .invoke("loadImageFromFile", file, "My sample image")
        .then((result) => {
          // console.log(result);
          this.zoom = 1;
          this.$refs.imageEditor.invoke("clearUndoStack");
        });
    },
    crop() {
      let croppedArea = this.$refs.imageEditor.invoke("getCropzoneRect");
      // console.log({ croppedArea });
      this.$refs.imageEditor.invoke("crop", croppedArea).then(() => {
        this.$refs.imageEditor.invoke("stopDrawingMode");
        this.zoomChange("in");
        // resizeEditor();
        // this.saveFile();
      });
    },
    saveFile() {
      var imageName = Date.now().toLocaleString();
      var dataURL = this.$refs.imageEditor.invoke("toDataURL");
      console.log({ dataURL });
      // var blob, type, w;
      // blob = base64ToBlob(dataURL);
      // type = blob.type.split("/")[1];
      // if (imageName.split(".").pop() !== type) {
      //   imageName += "." + type;
      // }
      let w = window.open();
      w.document.body.innerHTML = '<img src="' + dataURL + '">';

      // // Library: FileSaver - saveAs
      // saveAs(blob, imageName); // eslint-disable-line
    },
    startLineDrawing() {
      this.$refs.imageEditor.invoke("startDrawingMode", "LINE_DRAWING", {
        width: 5,
        color: "rgba(255, 0, 0, 0.5)",
      });
    },
    zoomChange(type) {
      let updated = this.zoom;
      if (type === "in") {
        if (updated <= this.maxZoom) this.zoom = updated + 0.25;
      } else {
        if (updated >= this.minZoom) this.zoom = updated - 0.25;
      }
      if (!this.loaded) {
        const canvas = document.querySelector("canvas");
        this.height = this.getNumber(canvas.style.maxHeight);
        this.width = this.getNumber(canvas.style.maxWidth);
        this.loaded = true;
      }
      this.resizeElement(
        document.querySelector(".tui-image-editor-canvas-container"),
        this.zoom,
        ["height", "width"],
        [this.height, this.width]
      );
      this.resizeElement(
        document.querySelector("div.tui-image-editor-canvas-container"),
        this.zoom,
        ["maxHeight", "maxWidth"],
        [this.height, this.width]
      );
      document.querySelectorAll("canvas").forEach((element) => {
        this.resizeElement(
          element,
          this.zoom,
          ["maxHeight", "maxWidth"],
          [this.height, this.width]
        );
      });
    },
  },
};
</script>

<style scoped>
.image-editor {
  width: 100%;
  height: 80%;
  color: green;
  text-align: center;
}
.input-image {
  display: none;
}
.PR {
  position: relative;
}

.zoom {
  position: absolute;
  left: 50%;
  color: black;
  z-index: 1000;
  display: flex;
  align-items: center;
}

.zoom span {
  cursor: pointer;
  font-size: 30px;
  margin: 0 20px;
}

.disabled {
  opacity: 0.5;
  pointer-events: none;
}
</style>