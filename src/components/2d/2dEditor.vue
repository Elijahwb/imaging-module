<template>
  <div class="image-editor">
    <tui-image-editor
      ref="imageEditor"
      :include-ui="useDefaultUI"
      :options="options"
    ></tui-image-editor>
    <button class="btn btn-primary pb-36" @click="addText('I love this')">
      Load Image
    </button>
    <button class="btn btn-primary pb-36" @click="startDrawing()">crop</button>
  </div>
</template>

<script >
import "tui-color-picker/dist/tui-color-picker.css";
import "tui-image-editor/dist/tui-image-editor.css";
import { ImageEditor } from "@toast-ui/vue-image-editor";

export default {
  name: "Editor2d",
  components: {
    "tui-image-editor": ImageEditor,
  },
  data() {
    return {
      useDefaultUI: false,
      options: {
        // for tui-image-editor component's "options" prop
        cssMaxWidth: 600,
        cssMaxHeight: 500,
        includeUI: {
          loadImage: {
            path: "data:image/png;base64,",
            name: "Blank",
          },
          menuBarPosition: "right",
        },
      },
    };
  },
  mounted() {
    // const editor = this.$refs.imageEditor;
    // console.log({ editor });

    // this.$refs.imageEditor
    //   .loadImageFromURL(
    //     "https://www.biltmoreperiodontics.com/wp-content/uploads/2021/06/teeth-in-skeleton-lowrez-1024x626.jpg",
    //     "SampleImage"
    //   )
    //   .then((objectProps) => {
    //     console.log("Image Loaded");
    //     console.log(objectProps.id);
    //   })
    //   .catch((error) => {
    //     console.log({ error });
    //   });
    // this.resizeEditor();
    this.loadImage();
  },
  methods: {
    loadImage() {
      this.$refs.imageEditor
        .invoke(
          "loadImageFromURL",
          "data:image/png;base64,",
          "My sample image"
        )
        .then((result) => {
          console.log(result);
          this.$refs.imageEditor.invoke("rotate", 10).then((status) => {
            console.log({ status });
          });
        });
    },
    resizeEditor() {
      this.$refs.imageEditor
        .invoke("resize", {
          imageSize: {
            newWidth: 800,
            newHeight: 800,
          },
          uiSize: { width: 800, height: 800 },
        })
        .then((result) => {
          console.log(result);
        });
    },
    addText(text) {
      this.$refs.imageEditor.invoke("addText", text, {
        styles: {
          fill: "red",
          fontSize: 20,
          fontWeight: "bold",
        },
        position: {
          x: 10,
          y: 10,
        },
      });
    },
    rotateClockwise() {
      this.$refs.imageEditor.invoke("rotate", 10).then((status) => {
        console.log({ status });
      });
    },
    rotateAntiClockwise() {
      this.$refs.imageEditor.invoke("rotate", -10).then((status) => {
        console.log({ status });
      });
    },
    addShape() {
      this.$refs.imageEditor
        .invoke("addShape", "rect", {
          fill: "red",
          stroke: "blue",
          strokeWidth: 3,
          width: 100,
          height: 200,
          left: 10,
          top: 10,
          isRegular: true,
        })
        .then((status) => {
          console.log({ status });
        });
    },
    crop() {
      this.$refs.imageEditor.invoke("getCropzoneRect").then((data) => {
        console.log({ data });
        // this.$refs.imageEditor.invoke("crop", data).then((status) => {
        //   console.log({ status });
        // });
      });
    },
    cameraZoom() {
      this.$refs.imageEditor.invoke("zoom", {}, 1, 1, 2.5).then((status) => {
        console.log({ status });
      });
    },
    getEditorCanvasSize() {
      this.$refs.imageEditor.invoke("getDrawingMode", {}).then((status) => {
        console.log({ status });
      });
    },

    startDrawing() {
      this.$refs.imageEditor.invoke("startDrawingMode", "FREE_DRAWING", {
        width: 5,
        color: "rgba(255, 0, 0, 0.5)",
      });
      // this.$refs.imageEditor
      //   .invoke("startDrawingMode", "FREE_DRAWING", {
      //     width: 5,
      //     color: "rgba(0, 0, 0, 0.5)",
      //   })
      //   .then((status) => {
      //     console.log({ status });
      //     // this.$refs.imageEditor.invoke("setBrush", {
      //     //   width: 8,
      //     //   color: "rgba(0, 0, 0, 0.5)",
      //     // });
      //   })
      //   .catch((message) => {
      //     console.log("error: ", message);
      //   });
    },
  },
};
</script>

<style scoped>
.image-editor {
  width: 100%;
  height: 100%;
  color: green;
  text-align: center;
}
</style>